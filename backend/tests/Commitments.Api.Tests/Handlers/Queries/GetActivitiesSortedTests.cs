...
public class GetActivitiesSortedTests
{
    ...
    public async Task Handle_ReturnsActivitiesOrderedByPerformedOnDescending(/* design 10 Slice A */)
    {
        ...
        ctx.Activities.Add(new Activity { ActivityId = oldId, BehaviourId = b.BehaviourId, PerformedOn = new DateTime(2026, 1, 1) });
        ctx.Activities.Add(new Activity { ActivityId = newId, BehaviourId = b.BehaviourId, PerformedOn = new DateTime(2026, 6, 1) });
        ctx.Activities.Add(new Activity { ActivityId = midId, BehaviourId = b.BehaviourId, PerformedOn = new DateTime(2026, 3, 1) });
