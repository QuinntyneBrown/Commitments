...
const SCREENSHOT =
  process.env.SCREENSHOT ?? `docs/bugs/screenshots/${SLUG}-actual-${VIEWPORT_W}.png`;
...
const heading = await page
  .locator('h1, h2, app-primary-header')
  .first()
  .textContent()
  .catch(() => null);
const url = page.url();

const dataTable = await page.locator('app-data-table').count();
const fab = await page.locator('button.mat-mdc-fab, button.mat-fab').count();
const fabIcon = await page
  .locator('button.mat-mdc-fab mat-icon, button.mat-fab mat-icon')
  .first()
  .textContent()
  .catch(() => null);
...
const dataTableClasses = await page.evaluate(() => {
  const el = document.querySelector('app-data-table');
  return el ? Array.from(el.classList) : [];
});
const dataTableBg = await page
  .locator('app-data-table')
  .first()
  .evaluate((el) => (el ? getComputedStyle(el).backgroundColor : 'none'))
  .catch(() => 'none');

await page.screenshot({ path: SCREENSHOT, fullPage: true });

const pageHtml = await page
  .locator('app-commitments-page')
  .first()
  .innerHTML()
  .catch(() => '<no app-commitments-page>');
...
const routerOutletSiblingTag = await page.evaluate(() => {
  ...
  return sib
    ? sib.tagName +
        (sib.id ? `#${sib.id}` : '') +
        (sib.className ? `.${sib.className.split(' ').join('.')}` : '')
    : null;
});
  ...
  JSON.stringify(
    {
      url,
      heading,
      counts: { dataTable, fab, sidenav, topbar, root: rootCount },
